...
int s21_div(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  ...
  s21_set_sign(&check_equal_1, 0);
  s21_decimal check_equal_2 = value_2;
  s21_set_sign(&check_equal_2, 0);
  sign_and_pow sc_1 = s21_parsing_sign_and_pow(value_1);
  sign_and_pow sc_2 = s21_parsing_sign_and_pow(value_2);

  if (s21_is_equal_null(check_zero))
    flag = 3;
  else if (s21_is_equal(check_equal_1, check_equal_2)) {
    *result = DECIMAL_1;
    s21_big_decimal res = s21_decimal_to_big_decimal(*result);
    s21_sign_for_res(sc_1, sc_2, &res);
    *result = s21_big_decimal_to_decimal(res);
  } else if (s21_is_equal_null(value_1))
    *result = DECIMAL_0;
  else {
    s21_big_decimal val_1 = s21_decimal_to_big_decimal(value_1);
    s21_big_decimal val_2 = s21_decimal_to_big_decimal(value_2);
    s21_big_decimal res = {0};
    sign_and_pow scale_1 = s21_parsing_sign_and_pow_big(val_1);
    sign_and_pow scale_2 = s21_parsing_sign_and_pow_big(val_2);

    if (scale_1.pow == scale_2.pow && scale_1.pow == 0) {
      s21_div_zero_pow_big(val_1, val_2, &res);

      if (res.bits[3] != 0 || res.bits[4] != 0 || res.bits[5] != 0 ||
          s21_get_power_big(res) > 28)
        s21_round_to_dec_precision(&res);
      else
        s21_remove_nonsignificant_zeros(&res);
      // устанавливаем знак в резалт
      s21_sign_for_res(sc_1, sc_2, &res);
      // установка флага переполнения на бесконечно большое или бесконечно малое
      s21_apply_flag_to_mauch_or_litle(&flag, res);
    } else {
      // 1. сделать оба числа с 0 степенью
      // 2. сделать деление биг децималов одинаковой степени
      s21_pow1_pow2_align(&val_1, &val_2, value_1, value_2);
      flag = s21_div_zero_pow_big(val_1, val_2, &res);
  ...
  if (flag) s21_decimal_to_null(result);
  return flag;
}
...
int s21_div_zero_pow_big(s21_big_decimal value_1, s21_big_decimal value_2,
  ...
  if (!flag) {
    ...
    sign_and_pow scale_1 = s21_parsing_sign_and_pow_big(res_left_half);
    sign_and_pow scale_2 = s21_parsing_sign_and_pow_big(res_right_half);
    s21_plus_plus_big(res_left_half, res_right_half, scale_1, scale_2, res_temp,
                      &flag);
    s21_set_power_big(res_temp, s21_get_power_big(res_right_half));
  }
  return flag;
}
    ...
    if (s21_is_greater_or_equal_big(float_part, divider)) {
      ...
      *result = bufer;
      ...
      while (s21_is_greater_or_equal_big(float_part, divider)) {
        ...
        s21_add_two_positiv_big(&bufer, *result, DECIMAL_1_BIG);
        *result = bufer;
        s21_big_decimal_to_null(&bufer);
      }
    } else {
...
s21_big_decimal s21_div_integer_part_big(s21_big_decimal value_1,
  ...
  int32_t bits_divisible = s21_count_bits_big(value_1);
  ...
  if (s21_is_greater_big(
    ...
    while (1) {
      // вытащили бит из делимого в флоат
      temp_bit = s21_get_bit_big(value_1, bits_divisible - 1 - i);
      float_part = s21_left_shift_big(float_part, 1);
      s21_set_bit_big(&float_part, 0, temp_bit);
      i++;
      ...
      if (s21_is_greater_or_equal_big(float_part, divider)) {
        ...
        *result = s21_left_shift_big(*result, 1);
        s21_set_bit_big(result, 0, 1);
        s21_big_decimal bufer = {0};
        s21_sub_big_small_big(float_part, divider, &bufer);
        s21_big_decimal_to_null(&float_part);
        float_part = bufer;
      } else {
        *result = s21_left_shift_big(*result, 1);
...
void s21_sign_for_res(sign_and_pow sc_1, sign_and_pow sc_2,
  ...
  if ((!sc_1.sign && !sc_2.sign) || (sc_1.sign && sc_2.sign))
    s21_set_sign_big(res, 0);
  else
    s21_set_sign_big(res, 1);
}