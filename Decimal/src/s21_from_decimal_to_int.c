...
int s21_from_decimal_to_int(s21_decimal src, int *dst) {
  ...
  int validity = s21_validation_check(src);
  ...
  if (validity || dst == NULL)
    ...
    if (scale != 0) {
      for (int i = 0; i < scale; i++) {
        s21_decimal my_num = s21_div_to_ten(src);
        src = my_num;
      }
...
s21_decimal s21_div_to_ten(s21_decimal src) {
  ...
  int index = s21_beginning(
      arr);  // index - первое вхождение 1 (откуда начинается число)

  int first = s21_first_5_bits(
      arr,
      index);  // берем первый 5 битов числа и записываем в переменную first

  int divisible = s21_finding_the_first_divisible(
