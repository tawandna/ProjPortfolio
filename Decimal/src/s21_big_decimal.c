...
int32_t s21_is_less_big(s21_big_decimal value_1, s21_big_decimal value_2) {
  int flag = 0;
  int sign_1 = s21_get_sign_big(value_1);  // какой знак
  int sign_2 = s21_get_sign_big(value_2);
  int count_1 = s21_get_power_big(value_1);  // степень
  int count_2 = s21_get_power_big(value_2);
  ...
  if (sign_1 == 1 && sign_2 == 0) {
    ...
    if (count_1 > count_2) {
      s21_apply_pow(&value_2, count_1);
    } else {
      s21_apply_pow(&value_1, count_2);
    }
...
void s21_from_big_decimal_to_str(s21_big_decimal value, char *dec_str) {
  ...
  s21_pow_to_str_dec(dec_number, dec_str, scale);
...
int s21_set_bit_big(s21_big_decimal *value, uint8_t bit_idx, uint8_t bit_val) {
  ...
  if (bit_idx > 192 || bit_val > 1) {
    ...
    if (bit_val != s21_get_bit_big(*value, bit_idx)) {
...
s21_big_decimal s21_left_shift_big(s21_big_decimal value, int n) {
  ...
  int higth_bit = s21_count_bits_big(res);

  for (; n > 0; n--) {
    higth_bit = s21_count_bits_big(res);
    for (int i = higth_bit - 1; i >= 0; i--) {
      // вытащили бит i записали его в i+1 место
      int bit = s21_get_bit_big(res, i);
      s21_set_bit_big(&res, i + 1, bit);
    }
    s21_set_bit_big(&res, 0, 0);
  }
...
s21_big_decimal s21_right_shift_big(s21_big_decimal value, int n) {
  ...
  for (int j = 0; j < n; j++) {
    ...
    s21_set_bit_big(&res, higth_bit - 1, 0);
  }
...
void s21_sub_big_small_big(s21_big_decimal value_1, s21_big_decimal value_2,
  ...
  for (int32_t j = 0; j <= 5; j++) {
    for (int32_t i = 31; i >= 0; i--) {
      ...
      s21_set_bit_big(result, j * 32 + (31 - i), bit_res);
    }
  }
}
...
int s21_mul_equal_pow_big(s21_big_decimal value_1, s21_big_decimal value_2,
  ...
  int add_res;
  int bits = s21_count_bits_big(value_2);
  for (int i = 0; i < bits; i++) {
    int current_bit = 1 & s21_right_shift_big(value_2, i).bits[0];
    if (current_bit) {
      tmp = s21_left_shift_big(value_1, i);
...
int32_t s21_add_two_positiv_big(s21_big_decimal *result,
  ...
  overflow_0 = s21_add_overflow(value_1.bits[0], value_2.bits[0]);

  for (int i = 1; i <= 5; i++) {
    (*result).bits[i] = value_1.bits[i] + value_2.bits[i];
    overflow_1 = s21_add_overflow(value_1.bits[i], value_2.bits[i]);
    if (overflow_0) {
      (*result).bits[i] += 1;
      if ((*result).bits[i] == 0) {
        overflow_1 = 1;
  ...
  int pow_1 = s21_get_power_big(value_1);
  int pow_2 = s21_get_power_big(value_2);
  int bufer_pow = pow_1 <= pow_2 ? pow_1 : pow_2;
  s21_big_decimal bufer = *result;
  s21_big_decimal temp = {0};
  s21_set_power_big(&bufer, bufer_pow);
...
int s21_get_sign_big(s21_big_decimal value) {
  return (value.bits[7] >> 31) & 1;
}

void s21_set_sign_big(s21_big_decimal *value, int sign) {
  if (sign != s21_get_sign_big(*value)) {
    if (sign == 1) {
