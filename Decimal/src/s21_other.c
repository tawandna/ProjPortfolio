...
int s21_negate(s21_decimal value, s21_decimal *result) {
  ...
  if (result == NULL)
    ...
    if (!flag) {
      *result = value;
      int bit = s21_get_sign(value);
      if (!s21_is_equal_null(value)) s21_set_sign(result, bit == 0 ? 1 : 0);
    }
  }
  return flag;
}
...
int s21_truncate(s21_decimal value, s21_decimal *result) {
  int error = 0;
  error = s21_validation_check(value);
  ...
  if (!error) {
    ...
    if (scale == 0)
      ...
      for (int i = scale; i > 0 && !(value.bits[0] < 10 && value.bits[1] == 0 &&
                                     value.bits[2] == 0);
           i--) {
        result_my_num = s21_div_to_ten(value);
        value = result_my_num;
        count_scale--;
      }
    }
...
int s21_floor(s21_decimal value, s21_decimal *result) {
  ...
  if (result == NULL)
    ...
    if (sign == 1)
      s21_add_two_positiv(
          &bufer, s21_is_equal_null(ostatok) == 0 ? DECIMAL_1 : DECIMAL_0,
          value);
    else
      bufer = value;
    s21_set_sign(&bufer, sign);
