...
int32_t s21_add(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  ...
  if (!scale_1.sign && !scale_2.sign) {
    s21_math_add(val_1, val_2, &res, scale_1, scale_2, &flag);
...
void s21_math_add(s21_big_decimal val_1, s21_big_decimal val_2,
  ...
  s21_plus_plus_big(val_1, val_2, scale_1, scale_2, res, flag);
  // применили большую степень к резул биг децимал
  s21_set_power_big(res, scale_1.pow > scale_2.pow ? scale_1.pow : scale_2.pow);
  // начали округление
...
void s21_plus_plus_big(s21_big_decimal value_1, s21_big_decimal value_2,
  ...
  s21_set_sign_big(&value_1, 0);
  s21_set_sign_big(&value_2, 0);
  if (scale_1.pow == scale_2.pow) {
    s21_set_power_big(result, scale_1.pow);
    *flag = s21_add_two_positiv_big(result, value_1, value_2);
  } else {
    // выравнивание степени
    if (scale_1.pow > scale_2.pow) {
      s21_apply_pow(&value_2, scale_1.pow);
    } else {
      s21_apply_pow(&value_1, scale_2.pow);
    }
    *flag = s21_add_two_positiv_big(result, value_1, value_2);
  }
}
...
void s21_apply_pow(s21_big_decimal *value, int32_t pow) {
  ...
  int now_pow = s21_get_power_big(*value);
  if (pow > now_pow) {
...
int32_t s21_add_overflow(uint32_t x, uint32_t y) {
  int32_t res = 0;
