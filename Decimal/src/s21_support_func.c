...
void s21_from_decimal_to_str(s21_decimal value, char *dec_str) {
  sign_and_pow scale = s21_parsing_sign_and_pow(value);

  char binary_number[97] = {'\0'};
  char dec_number[31] = {'\0'};
  // делаем строку двоичного числа
  s21_str_bin(binary_number, &value);
...
s21_decimal s21_div_integer_part(s21_decimal value_1, s21_decimal value_2,
  ...
  int32_t bits_divisible = s21_count_bits(value_1);
  int32_t temp_bit = 0;
  ...
  if (s21_is_greater(
    ...
    while (1) {
      ...
      temp_bit = s21_get_bit(value_1, bits_divisible - 1 - i);
      float_part = s21_left_shift(float_part, 1);
      s21_set_bit(&float_part, 0, temp_bit);
      i++;
      ...
      if (s21_is_greater_or_equal(float_part, divider)) {
        ...
        *result = s21_left_shift(*result, 1);
        s21_set_bit(result, 0, 1);
        s21_decimal bufer = {0};
        s21_sub(float_part, divider, &bufer);
        s21_decimal_to_null(&float_part);
        float_part = bufer;
      } else {
        *result = s21_left_shift(*result, 1);
...
int s21_count_bits(s21_decimal value) {
  int res = 96;
  while (s21_get_bit(value, res - 1) == 0 && res > 0) {
    res--;
  }
...
s21_decimal s21_right_shift(s21_decimal value, int n) {
  ...
  int higth_bit = s21_count_bits(res);

  for (int j = 0; j < n; j++) {
    higth_bit = s21_count_bits(res);
    for (int i = 1; i <= higth_bit - 1; i++) {
      // вытащили бит i записали его в i-1 место
      int bit = s21_get_bit(res, i);
      s21_set_bit(&res, i - 1, bit);
    }
    s21_set_bit(&res, higth_bit - 1, 0);
  }
  // s21_decimal res = value;
...
int s21_set_bit(s21_decimal *value, uint8_t bit_idx, uint8_t bit_val) {
  ...
  if (bit_idx > 127 || bit_val > 1) {
    ...
    if (bit_val != s21_get_bit(*value, bit_idx)) {
